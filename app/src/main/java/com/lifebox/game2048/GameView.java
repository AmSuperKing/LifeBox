...
//      Toast.makeText(getContext(), "向下滑动了", 0).show();
    ...
    private void check(){
        ...
        if (complete){
            new AlertDialog.Builder(getContext()).setTitle("啦啦啦~").setMessage("游戏结束").setPositiveButton("再玩一局", new DialogInterface.OnClickListener() {
                @Override
        ...
        for (int x = 0;x < 4;x++){
            for(int y = 0;y < 4;y++){
                if (cardsMap[x][y].getNum()==2048){
                    new AlertDialog.Builder(getContext()).setTitle("耶耶耶~").setMessage("你赢了!").setPositiveButton("还玩吗？", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            startGame();
                        }
